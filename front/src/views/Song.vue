<template>
  ...
  <div
    v-else-if="!marks.isLoading && songsData.currentSong"
    class="flex items-center flex-col mt-6"
  >
    <div class="flex items-center">
      <img
        class="rounded-lg w-48 h-48 md:w-72 md:h-72"
    ...
    <Text class="mt-2 mb-2 text-xl" :type="'title'">
      {{ songsData.currentSong.title }}
    </Text>
    <Text class="text-secondary" :type="'subtitle'">
      {{ songsData.currentSong.album }}
    </Text>
    <Player v-if="songsData.currentAudio" :audio="songsData.currentAudio" />
  </div>
  <div v-else>
    <Text :type="'title'" class="text-center">
      Upsss! Problem with loading this song :-(
    </Text>
  </div>
</template>
