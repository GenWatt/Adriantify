<template>
  <li
    v-if="computedSong"
    class="rounded-lg flex justify-between my-1 transition-colors hover:bg-secondary"
  >
    <router-link
      class="flex w-full cursor-pointer"
      :to="{ name: 'Song', params: { id: computedSong._id } }"
    >
      <img
      ...
      <Text
        v-if="duration"
        :type="'subtitle'"
        :class="remove || edit ? 'self-center' : ''"
        >{{ computedSong.time }}</Text
      >
    </router-link>
    <div class="flex items-center">
      ...
      <div v-if="add" :class="buttonClass">
        <PlusIcon
          ...
          class="pl-2 w-6 h-6"
          @click="() => handleAdd(computedSong as SongType)"
        />
      </div>
    </div>
    <div class="flex items-center">
      ...
      <div
        v-if="remove"
        @click="() => handleRemove(computedSong as SongType)"
        :class="buttonClass"
...
const buttonClass =
  "flex justify-center  items-center hover:scale-110 cursor-pointer transition-transform";
